...
const ExploreMore = () => {
    ...
    return (
        <motion.div
            ref={ref}
            className="bg-[#14141F] py-20 text-center px-4"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: isInView ? 1 : 0, y: isInView ? 0 : 50 }} // Entry animation
            transition={{ duration: 0.5 }}
        >
            <h4 className="text-4xl md:text-4xl font-bold mb-2 text-cyan-500">
                Do you want to see more about the blogs I wrote?
            </h4>
            <p className="text-lg md:text-xl mb-6 max-w-6xl mx-auto text-gray-200">
                Explore a wide range of topics and insights from my writing journey.
            </p>
            <Link href="/#blogs"> {/* Use the hash to link to the ID */}
                <motion.button
                    className="bg-cyan-400 text-white px-8 py-4 rounded-lg hover:bg-blue-500 transition-all duration-300 transform hover:scale-105"
